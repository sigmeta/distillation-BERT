...
class BertForHiddenDistill(BertPreTrainedModel):
    """BERT model ("Bidirectional Embedding Representations from a Transformer").
    ...
    """
    def __init__(self, config):
        super(BertForHiddenDistill, self).__init__(config)
        self.bert = BertModel(config)
        self.reform=nn.Linear(config.hidden_size,768)
        self.apply(self.init_bert_weights)
    ...
    def forward(self, input_ids, token_type_ids=None, attention_mask=None, targets=None):
        ...
        if targets is not None:
            loss_fct = nn.MSELoss(size_average=True)
            reformed_output=self.reform(sequence_output)
            loss=loss_fct(reformed_output,targets)
            return loss
...
class BertForPPL(BertPreTrainedModel):
    """BERT model with pre-training heads.
    This module comprises the BERT model followed by the two pre-training heads:
        - the masked language modeling head, and
        - the next sentence classification head.
    ...
    """
    def __init__(self, config):
        super(BertForPPL, self).__init__(config)
        self.bert = BertModel(config)
        self.cls = BertPreTrainingHeads(config, self.bert.embeddings.word_embeddings.weight)
        #self.apply(self.init_bert_weights)
...
class BertForScore(BertPreTrainedModel):
    """BERT model with pre-training heads.
    This module comprises the BERT model followed by the two pre-training heads:
        - the masked language modeling head, and
        - the next sentence classification head.
    ...
    """
    def __init__(self, config):
        super(BertForScore, self).__init__(config)
        self.bert = BertModel(config)
        self.cls = BertPreTrainingHeads(config, self.bert.embeddings.word_embeddings.weight)
        #self.apply(self.init_bert_weights)
...
class BertForPolyphonyMulti(BertPreTrainedModel):
    ...
    def forward(self, input_ids, attention_mask=None, labels=None, token_type_ids=None, logit_masks=None,
        ...
        if cal_loss and targets is None:
            ...
            nll_loss = loss_fct(logits.view(-1, self.num_labels), labels.view(-1))
...
class BertForPolyphonyInference(BertPreTrainedModel):
    ...
    def forward(self, input_ids):
        ...
        sequence_output, pooled_output = self.bert(input_ids, None, None, output_all_encoded_layers=False)
        output = self.dropout(sequence_output)
        logits = self.classifier(output)
...
class PolyphonyLSTM(BertPreTrainedModel):
    ...
    def __init__(self, config, num_labels):
        ...
        self.hidden_size=config.hidden_size
        print(num_labels)
        self.embed=nn.Embedding(config.vocab_size,config.hidden_size,padding_idx=-1)
        #self.bert = BertModel(config)
        self.dropout = nn.Dropout(config.hidden_dropout_prob)
        self.bilstm = nn.LSTM(config.hidden_size, config.hidden_size//2, num_layers=1, batch_first=True,
                              dropout=0, bidirectional=True)
    ...
    def forward(self, input_ids, attention_mask=None, labels=None, token_type_ids=None, logit_masks=None, cal_loss=True, weight=None):
        ...
        #hidden = (torch.autograd.Variable(torch.zeros(2, input_ids.size()[0], self.hidden_size)),
        #          torch.autograd.Variable(torch.zeros(2, input_ids.size()[0], self.hidden_size)))
        embed_output=self.embed(input_ids)
        #output = self.dropout(embed_output)
        output,_ = self.bilstm(embed_output)
        output = self.dropout(output)
        logits = self.classifier(output)
        #print(logit_masks.size(),labels.size())
        if logit_masks is not None:
            ...
            for j in range(labels.size()[1]):
                if labels[i,j]!=-1:
                    ...
                    if output_lstm is None:
                        output_lstm=self.bilstm(output[i,max(0,j-1):j+2,:].unsqueeze(0))[0][0,p,:].unsqueeze(0)
                    else:
...
class BertForAbbrVocab(BertPreTrainedModel):
    ''''''
    def __init__(self, config):
        super(BertForAbbrVocab, self).__init__(config)
        self.bert = BertModel(config)
        self.cls = BertPreTrainingHeads(config, self.bert.embeddings.word_embeddings.weight)
        #self.apply(self.init_bert_weights)

    def forward(self, input_ids, token_type_ids=None, attention_mask=None, masked_lm_labels=None,
                next_sentence_label=None, cal_loss=True, labels=None, targets=None):
        sequence_output, pooled_output = self.bert(input_ids, token_type_ids, attention_mask,
                                                   output_all_encoded_layers=False)
        prediction_scores, seq_relationship_score = self.cls(sequence_output, pooled_output)
        score=prediction_scores[masked_lm_labels.ne(-1)]
        if cal_loss:
            self.compute_loss(score,targets)
        #assert score.size(1)==1
        else:
            return score.squeeze()
