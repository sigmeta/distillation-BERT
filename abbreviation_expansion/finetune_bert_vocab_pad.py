...
from pytorch_pretrained_bert.modeling import BertForAbbrPad
...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, label_ids, label_pos, char, candidates, label, unique_id):
...
class DataProcessor(object):
    ...
    def get_train_examples(self, data_dir):
        ...
        train_list=[]
        with open(os.path.join(data_dir, "train.txt"), encoding='utf8') as f:
            for line in f:
                if not line.strip():
                    continue
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        if is_test:
            ...
            if position==None:
                ...
                bad_count+=1
                continue
            if position>max_seq_length-2-max_replacement_length:
                bad_count += 1
                continue
        else:
        ...
        replacement+=['[MASK]']*(max_replacement_length-len(replacement))
        ...
        label_ids[position + 1:position+1+len(label)] = label
...
def get_result(logit,pos,candidates,label,tokenizer):
    best_score=0;best_can=None
    logit = logit[pos:pos + 12]
    for can in candidates:
        score=0
        can_ids=tokenizer.convert_tokens_to_ids(tokenizer.tokenize(can))
    ...
    if best_can.lower()==label.lower():
        res=1
    else:
        res=0
    return float(best_can),int(res)
...
def main():
    ...
    if args.do_train:
        train_features, masks, weight = convert_examples_to_features(
            train_examples, label_list, args.max_seq_length, tokenizer,abex, args.ratio)
        ...
        for ep in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            if args.eval_every_epoch:
                ...
                config = BertConfig(output_config_file)
                model_eval = BertForAbbrPad(config, num_labels=num_labels)
                ...
                all_ids = torch.tensor([f.unique_id for f in eval_features], dtype=torch.long)
                all_candidates = [f.candidates for f in eval_features]
                all_labels = [f.label for f in eval_features]
                eval_data = TensorDataset(all_input_ids, all_input_mask, all_label_ids, all_label_poss, all_ids)
                ...
                out_list = []
                # masks = masks.to(device)
                for input_ids, input_mask, label_ids, label_poss, all_ids in tqdm(eval_dataloader, desc="Evaluating"):
                    ...
                    with torch.no_grad():
                        tmp_eval_loss = model(input_ids, attention_mask=input_mask, labels=label_ids, cal_loss=True)
                        logits = model(input_ids, attention_mask=input_mask, labels=label_ids, cal_loss=False)
                    # print(logits.size())
                    logits = logits.detach().cpu().numpy()
                    label_ids = label_ids.to('cpu').numpy()
                    for id in all_ids:
                        tmp_out, tmp_res = get_result(logits[id], label_poss[id], all_candidates[id], all_labels[id],
                                                      tokenizer)
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for input_ids, input_mask, label_ids, label_poss, all_ids in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            label_ids = label_ids.to('cpu').numpy()
            for id in all_ids:
                tmp_out, tmp_res = get_result(logits[id], label_poss[id], all_candidates[id], all_labels[id], tokenizer)
                res_list.append(tmp_res)
                out_list.append(tmp_out)
