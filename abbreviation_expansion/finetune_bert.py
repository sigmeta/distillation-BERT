...
from pytorch_pretrained_bert.modeling import BertForAbbr
...
def main():
    ...
    if args.do_train:
        ...
        all_targets=torch.tensor([f.targets for f in train_features], dtype=torch.float)
        train_data = TensorDataset(all_input_ids, all_input_mask, all_label_ids, all_label_poss, all_targets)
        if args.local_rank == -1:
        ...
        for ep in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                # print(masks.size())
                loss = model(input_ids, input_mask, label_ids, logit_masks=masks, weight=weight, hybrid_mask=None, targets=targets)
                if n_gpu > 1:
            ...
            if args.eval_every_epoch:
                ...
                for input_ids, input_mask, label_ids, label_poss, targets in tqdm(eval_dataloader, desc="Evaluating"):
                    ...
                    with torch.no_grad():
                        tmp_eval_loss = model_eval(input_ids, input_mask, label_ids, logit_masks=masks, hybrid_mask=None, targets=targets)
                        logits = model_eval(input_ids, input_mask, label_ids, logit_masks=masks, cal_loss=False, hybrid_mask=None, targets=targets)
                    # print(logits.size())
