...
def accuracy_list(out, labels, positions,label_list):
    ...
    for i, p in enumerate(positions):
        ...
        if outputs[i, p] == labels[i, p]:
            ...
            res.append(0)
        ps.append(label_list[outputs[i,p]])
    return res,ps
...
def main():
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for input_ids, input_mask, label_ids, label_poss in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            res,prediction=accuracy_list(logits,label_ids,label_poss,label_list)
            res_list += res
            pre_list+=prediction
            eval_loss += tmp_eval_loss.mean().item()
        ...
        output_prelist_file = os.path.join(args.output_dir, args.test_set + "prelist.json")
        ...
        with open(output_prelist_file,'w',encoding='utf8') as f:
            f.write(json.dumps(pre_list,ensure_ascii=False))
