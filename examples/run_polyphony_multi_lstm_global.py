...
from pytorch_pretrained_bert.modeling import BertForPolyphonyMultiLSTM, BertConfig, WEIGHTS_NAME, CONFIG_NAME, OPTIMIZER_NAME
...
def main():
    ...
    if os.path.exists(args.output_dir) and os.listdir(args.output_dir) and args.do_train:
        ...
        if os.path.exists(os.path.join(args.output_dir, WEIGHTS_NAME + '_' + str(max_epoch))):
            ...
            model = BertForPolyphonyMultiLSTM(config, num_labels=num_labels)
        ...
        if args.state_dir and os.path.exists(args.state_dir):
            ...
            state_dict=None
        model = BertForPolyphonyMultiLSTM.from_pretrained(args.bert_model,
                                                  cache_dir=cache_dir,
    ...
    if args.do_train:
        ...
        for ep in trange(max_epoch+1,int(args.num_train_epochs), desc="Epoch"):
            ...
            if args.eval_every_epoch:
                ...
                config = BertConfig(output_config_file)
                model_eval = BertForPolyphonyMultiLSTM(config, num_labels=num_labels)
