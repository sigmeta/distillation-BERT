...
class InputFeatures(object):
    ...
    def __init__(self, input_ids, input_mask, label_ids, label_pos, char, type_ids):
...
def main():
    ...
    if args.do_train:
        ...
        train_data = TensorDataset(all_input_ids, all_input_mask, all_label_ids, all_label_poss, all_type_ids)
        if args.local_rank == -1:
        ...
        for ep in trange(int(args.num_train_epochs), desc="Epoch"):
            ...
            for step, batch in enumerate(tqdm(train_dataloader, desc="Iteration")):
                ...
                input_ids, input_mask, label_ids, label_poss, type_ids = batch
                # print(masks.size())
                loss = model(input_ids, input_mask, label_ids, logit_masks=masks, token_type_ids=type_ids)
                if n_gpu > 1:
            ...
            if args.eval_every_epoch:
                ...
                for input_ids, input_mask, label_ids, label_poss, type_ids in tqdm(eval_dataloader, desc="Evaluating"):
                    ...
                    with torch.no_grad():
                        tmp_eval_loss = model_eval(input_ids, input_mask, label_ids, logit_masks=masks, token_type_ids=type_ids)
                        logits = model_eval(input_ids, input_mask, label_ids, logit_masks=masks, cal_loss=False, token_type_ids=type_ids)
                    # print(logits.size())
    ...
    if args.do_eval and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
        ...
        for input_ids, input_mask, label_ids, label_poss, type_ids in tqdm(eval_dataloader, desc="Evaluating"):
            ...
            with torch.no_grad():
                tmp_eval_loss = model(input_ids, input_mask, label_ids, logit_masks=masks, token_type_ids=type_ids)
                logits = model(input_ids, input_mask, label_ids, logit_masks=masks, cal_loss=False, token_type_ids=type_ids)
            # print(logits.size())
            logits = logits.detach().cpu().numpy()
            label_ids = label_ids.to('cpu').numpy()
